...
int brainfuck_execute_loop_start(struct brainfuck_state *state, int len, int pos, char *src)
{
  ...
  if(state->memory_buffer[state->memory_pointer_offset])
  {
    state->loop_buffer[state->loop_buffer_size] = pos;
    state->loop_buffer_size ++;
  }else{
    int cnt = 0;
...
int brainfuck_execute_loop_end(struct brainfuck_state *state, int pos)
{
  //printf("%d\n",state->memory_buffer[state->memory_pointer_offset]);
  //if(state->memory_buffer[state->memory_pointer_offset] != 0){
  pos = state->loop_buffer[state->loop_buffer_size - 1] - 1;
  state->loop_buffer_size --;
  //}
...
bool brainfuck_main(struct brainfuck_state* state, char* src,
  ...
  for(int i = 0; i < len; ++i)
  {
    ...
    else if (src[i] == BRAINFUCK_TOKEN_LOOP_END) i = brainfuck_execute_loop_end(state, i);
    //else return false;
  }
  return true;
}
